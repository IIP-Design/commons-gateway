...
var latinMap = map[string]string{
	"Á": "A",
	"Ă": "A",
	"Ắ": "A",
	"Ặ": "A",
...
func replaceLatinChar(char string) string {
	val, ok := latinMap[char]
	if ok {
		return val
	} else {
		return char
...
func replaceLatinCharacters(value string) string {
	return LATIN_REGEX.ReplaceAllStringFunc(value, replaceLatinChar)
}

func removeIllegalCharacters(value string) string {
...
func DefaultKeySanitizer(objectKey string) string {
	return SanitizeObjectKey(objectKey, "-")
}

func TimestampObjectKey(objectKey string) string {
	ts := time.Now().Unix()
	return TIMESTAMP_REGEX.ReplaceAllString(objectKey, fmt.Sprintf("$1-%d.$2", ts))
}
