...
// TODO: Document
func SaveInvite(adminEmail string, guestEmail string, expires time.Time, hash string, salt string, setPending bool) error {
	var err error
	...
	if setPending {
		insertInvite := `INSERT INTO invites( invitee, proposer, pending, date_invited, pass_hash, salt, expiration ) VALUES ($1, $2, $3, $4, $5, $6, $7);`
		_, err = pool.Exec(insertInvite, guestEmail, adminEmail, setPending, currentTime, hash, salt, expires)
	} else {
		insertInvite := `INSERT INTO invites( invitee, inviter, pending, date_invited, pass_hash, salt, expiration ) VALUES ($1, $2, $3, $4, $5, $6, $7);`
		_, err = pool.Exec(insertInvite, guestEmail, adminEmail, setPending, currentTime, hash, salt, expires)
	}
